...
Summary of what each script for
- ```preprocessing.py```: Handles data loading, and preprocessing
- ```config.py```: Contains configuration settings like hyperparameters and device setup.
- ```embedding_loader.py```: Functions for loading GloVe embeddings.
- ```model_builder.py```: Defines the BILSTM/BERT architecture.
- ```train_model.py```: Contains the functions to initialize and train the architectures.
- ```evaluate_model.py```: Functions for architecture evaluation.
- ```predict.py```: Includes functions for making predictions and plotting results.
- ```utils.py```: Utility functions used across multiple modules.
- ```main.py```: The main script to run the overall process.
